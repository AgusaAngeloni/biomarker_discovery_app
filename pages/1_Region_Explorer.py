...
def make_region_scatter(
    ...
    labels = {
        "mean_hi": "Mean HI index in region",
...
st.sidebar.header("Expression filter")
apply_expression_filter = st.sidebar.checkbox("Apply expression filter", value=False)
max_mean_spearman_r = st.sidebar.slider(
    "Maximum mean Spearman r",
    -1.0,
    1.0,
    -0.06,
    0.01,
...
with m1:
    st.metric("Qualifying CpGs", f"{cpgs['site_id'].nunique():,}")
with m2:
    st.metric("Gene regions", f"{regions['gene_region_id'].nunique():,}")
with m3:
...
fig_expression = make_region_scatter(
    ...
    title=f"{tumor_type} - Expression Plot",
    size_label="Expression signal",
    size_max=45,
)
st.plotly_chart(
    fig_expression,
    use_container_width=True,
    config=plot_svg_config(
        filename=f"{tumor_type} - Expression Plot",
        height=700,
        width=1400,
    ),
)
...
st.info(
    ...
    "(100 × max(0, -mean_spearman_r) × n_qualifying_sites)."
)
