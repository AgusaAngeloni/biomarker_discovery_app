...
def plot_svg_config(filename: str, height: int = 700, width: int = 1400) -> dict:
    """Plotly toolbar SVG export configuration for on-screen plots."""
    return {
...
def make_large_font_export_figure(
    ...
    for axis_name in ("xaxis", "yaxis", "yaxis2", "xaxis2"):
    ...
    if getattr(fig_export.layout, 'annotations', None):
        ...
        for ann in fig_export.layout.annotations:
            ...
            font = ann_json.get('font', {}) or {}
            font['size'] = annotation_size
            ann_json['font'] = font
            updated_annotations.append(ann_json)
        fig_export.update_layout(annotations=updated_annotations)
...
def show_large_font_svg_export(
    ...
    """Show a second Plotly figure with larger fonts for toolbar SVG export."""
    ...
    with st.expander(expander_label, expanded=False):
        st.caption(
            ...
            "The SVG keeps the same plot style and colors; only the font is larger."
        )
        st.plotly_chart(
            fig_export,
...
if enable_large_font_svg_export:
    show_large_font_svg_export(
        fig2,
        filename=f"{gene}_{tumor_type}_larger_font",
        expander_label="Methylation gene profile — SVG export with larger font",
        height=600,
        width=1400,
    )
...
if enable_large_font_svg_export:
    show_large_font_svg_export(
        fig3,
        filename=f"{gene}_{tumor_type}_methylation_expression_larger_font",
