...
def normalize_score_weights(
    methylation_quality: float,
    sequence_context: float,
    regional_support: float,
    expression_signal: float,
    include_expression: bool,
) -> dict[str, float]:
    """
    Normalize user-defined score weights so the final region score remains
    on a 0-100 scale.

    The weights are applied in Streamlit/Pandas after the SQL query has already
    loaded the candidate CpGs. This keeps score tuning dynamic without creating
    new database tables or rerunning expensive SQL work.
    """
    weights = {
        "methylation_quality": max(float(methylation_quality), 0.0),
        "sequence_context": max(float(sequence_context), 0.0),
        "regional_support": max(float(regional_support), 0.0),
        "expression_signal": max(float(expression_signal), 0.0) if include_expression else 0.0,
    }
    ...
    if total <= 0:
        if include_expression:
            weights = {
                "methylation_quality": 0.40,
                "sequence_context": 0.25,
                "regional_support": 0.20,
                "expression_signal": 0.15,
            }
        else:
            weights = {
                "methylation_quality": 0.45,
                "sequence_context": 0.30,
                "regional_support": 0.25,
                "expression_signal": 0.00,
            }
        total = sum(weights.values())
...
def aggregate_gene_regions(
    ...
    # ========================================================
    # The final score is computed in memory after the SQL query.
    # This makes the score tunable from Streamlit without creating
    # new database tables or rerunning expensive SQL calculations.
    #
    # Components:
    # - methylation_quality: high Δβ, high HI, low background methylation.
    # - sequence_context: tumor-independent DNA sequence score, scaled 0-1.
    # - regional_support: CpG support using log-scaled site count and the
    #   fraction of manifest CpGs that pass filters.
    # - expression_signal: optional inverse methylation-expression association.
    # ========================================================

    region["sequence_context"] = (
        pd.to_numeric(region["sequence_score"], errors="coerce")
        .fillna(0)
        .clip(0, 100)
        / 100
    )
    ...
    region["hi_norm"] = (
        pd.to_numeric(region["mean_hi"], errors="coerce")
        .fillna(0)
        .clip(0, 5)
        / 5
    )
    ...
    low_normal = (
        1 - pd.to_numeric(region["mean_normal_median"], errors="coerce")
    ).clip(0, 1).fillna(0)

    low_pan_normal = (
        1 - pd.to_numeric(region["mean_pan_normal_median"], errors="coerce")
    ).clip(0, 1).fillna(0)

    low_pan_tumor = (
        1 - pd.to_numeric(region["mean_pan_tumor_median"], errors="coerce")
    ).clip(0, 1).fillna(0)

    low_leukocyte = (
        1 - pd.to_numeric(region["mean_leukocyte_median"], errors="coerce")
    ).clip(0, 1).fillna(0)
    ...
    region["methylation_quality"] = (
        pd.to_numeric(region["mean_delta"], errors="coerce").fillna(0).clip(0, 1)
        * region["hi_norm"]
        * region["specificity_score"]
    )

    n_sites = (
        pd.to_numeric(region["n_qualifying_sites"], errors="coerce")
        .fillna(0)
        .clip(lower=0)
    )
    max_sites = float(n_sites.max()) if len(n_sites) else 0.0

    if max_sites > 0:
        region["log_site_support"] = np.log1p(n_sites) / np.log1p(max_sites)
    else:
        region["log_site_support"] = 0.0

    region["regional_support"] = (
        0.50 * region["log_site_support"]
        + 0.50 * pd.to_numeric(
            region["fraction_qualifying_sites"], errors="coerce"
        ).fillna(0).clip(0, 1)
    ).clip(0, 1)

    region["expression_signal"] = (
        -pd.to_numeric(region["mean_spearman_r"], errors="coerce")
    ).clip(lower=0).fillna(0).clip(0, 1)

    region["expression_size"] = (region["expression_signal"] ** 2 + 0.01).astype(float)

    # Keep the previous multiplicative score as a diagnostic/reference column,
    # but do not use it as the final score.
    ...
    weights = normalize_score_weights(
        score_weights.get("methylation_quality", 0.45),
        score_weights.get("sequence_context", 0.30),
        score_weights.get("regional_support", 0.25),
        score_weights.get("expression_signal", 0.0),
        include_expression=apply_expression_filter,
    )

    region["score_methylation_component"] = (
        100 * weights["methylation_quality"] * region["methylation_quality"]
    )
    region["score_sequence_component"] = (
        100 * weights["sequence_context"] * region["sequence_context"]
    )
    region["score_support_component"] = (
        100 * weights["regional_support"] * region["regional_support"]
    )

    if apply_expression_filter:
        region["score_expression_component"] = (
            100 * weights["expression_signal"] * region["expression_signal"]
        )
    else:
        region["score_expression_component"] = 0.0

    region["final_region_score"] = (
        region["score_methylation_component"]
        + region["score_sequence_component"]
        + region["score_support_component"]
        + region["score_expression_component"]
    ...
    region["final_region_score_size"] = region["final_region_score"].fillna(0).clip(lower=0.01)
...
st.sidebar.header("Dynamic Region Score")
st.sidebar.caption(
    "Weights are normalized automatically. Changing these sliders only recalculates "
    "the score in memory; it does not create or query new database tables."
)

default_methylation_weight = 0.40 if apply_expression_filter else 0.45
default_sequence_weight = 0.25 if apply_expression_filter else 0.30
default_support_weight = 0.20 if apply_expression_filter else 0.25
default_expression_weight = 0.15 if apply_expression_filter else 0.00

raw_methylation_weight = st.sidebar.slider(
    "Methylation quality weight",
    0.0,
    1.0,
    default_methylation_weight,
    0.05,
)
raw_sequence_weight = st.sidebar.slider(
    "Sequence context weight",
    0.0,
    1.0,
    default_sequence_weight,
    0.05,
)
raw_support_weight = st.sidebar.slider(
    "Regional support weight",
    0.0,
    1.0,
    default_support_weight,
    0.05,
)
raw_expression_weight = st.sidebar.slider(
    "Expression association weight",
    0.0,
    1.0,
...
score_weights = normalize_score_weights(
    methylation_quality=raw_methylation_weight,
    sequence_context=raw_sequence_weight,
    regional_support=raw_support_weight,
    expression_signal=raw_expression_weight,
    include_expression=apply_expression_filter,
)

st.sidebar.caption(
    "Normalized weights: "
    f"methylation={score_weights['methylation_quality']:.2f}, "
    f"sequence={score_weights['sequence_context']:.2f}, "
    f"support={score_weights['regional_support']:.2f}, "
    f"expression={score_weights['expression_signal']:.2f}"
)
...
regions = aggregate_gene_regions(
    cpgs,
    apply_expression_filter=apply_expression_filter,
...
if score_plot_df.empty:
    ...
    fig_score = make_region_scatter(
        ...
        title=f"{tumor_type} - Region-level candidates sized by dynamic region score",
        size_label="Final region score",
        size_max=55,
    ...
    st.plotly_chart(
        ...
        use_container_width=True,
        config=plot_svg_config(
            filename=f"{tumor_type} - Region-level candidates sized by dynamic region score",
            height=700,
            width=1400,
        ),
    )
...
region_cols = [
    ...
    "chr",
    "final_region_score",
    "score_methylation_component",
    "score_sequence_component",
    "score_support_component",
    "score_expression_component",
    "methylation_quality",
    "sequence_context",
    "regional_support",
    "specificity_score",
...
st.info(
    "Dynamic score: final_region_score = 100 × "
    "(w_methylation × methylation_quality + "
    "w_sequence × sequence_context + "
    "w_support × regional_support + "
    "w_expression × expression_signal). "
    "The number of qualifying CpGs is included through log_site_support and "
    "fraction_qualifying_sites, so it contributes as regional support without "
    "dominating the score linearly."
)
